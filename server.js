...
// Capacitor-specific headers and CORS
app.use((req, res, next) => {
    const isCapacitor = req.headers['x-capacitor'] === 'true' || 
                       req.headers.origin?.startsWith('capacitor://');
    
    if (isCapacitor) {
        console.log('🔧 Capacitor Request Detected');
        
        // More permissive headers for Capacitor
        res.header('Access-Control-Allow-Origin', '*');
        res.header('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
        res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, X-Capacitor, X-Session-ID, X-Device-Fingerprint');
        res.header('Access-Control-Expose-Headers', 'Set-Cookie, X-Session-ID, Authorization');
        res.header('Access-Control-Allow-Credentials', 'true');
        
        // Bypass security for preflight
        if (req.method === 'OPTIONS') {
            return res.sendStatus(200);
        }
        
        // Ensure session exists for Capacitor
        if (!req.session.initialized) {
            req.session.initialized = true;
            req.session.capacitorDevice = true;
        }
    }
    next();
});
...
// Enhanced iOS session recovery endpoint
app.post('/api/ios-session-recovery', async (req, res) => {
    console.log('📱 iOS Session Recovery Request');
    
    try {
        const { email, dbName, accessToken, deviceFingerprint } = req.body;
        ...
        if (accessToken) {
            try {
                ...
                console.log('Token verification failed, continuing with session check...');
            }
        }
        
        // If we have an existing session with user, use it
        if (req.session?.user) {
            console.log('✅ Using existing session:', req.session.user.email);
        ...
        // If we have credentials, try to restore session
        if (email && dbName) {
            ...
            // No credentials, just return current session state
            res.json({
                success: true,
                sessionId: req.sessionID,
                hasUser: !!req.session?.user,
                user: req.session?.user,
                message: 'Session state returned'
            });
        }
        
    } catch (error) {
        console.error('iOS session recovery error:', error);
        res.status(500).json({ 
            success: false, 
...
// Find this section in your server code
const sessionStore = new MySQLStore({
    ...
    expiration: 86400000, // CHANGE THIS: 24 Hours (matches your cookie maxAge)
    clearExpired: true
}, mainPool);
...
// Enhanced root route with iOS detection
app.get('/', (req, res) => {
    const userAgent = req.headers['user-agent'] || '';
    const isIOS = /iPhone|iPad|iPod/i.test(userAgent);
    const isCapacitor = req.headers['x-capacitor'] === 'true';
    ...
    // Always set session cookie for iOS/Capacitor
    if (req.sessionID && (isIOS || isCapacitor)) {
        res.cookie('solura.session', req.sessionID, {
            maxAge: 24 * 60 * 60 * 1000,
            httpOnly: false,
            secure: false,
        ...
        console.log('📱 iOS/Capacitor session cookie set');
    }
...
app.get('/api/ios-validate-session', (req, res) => {
    console.log('📱 iOS Session Validation');
    ...
    if (req.session?.user) {
        ...
        console.log('❌ iOS session validation failed');
        res.status(401).json({
            valid: false,
            device: 'ios',
...
function isAuthenticated(req, res, next) {
    ...
    if (req.session?.user && req.session.user.email) {
        // iPad/Mobile keep-alive
        if ((req.isIPad || req.isMobileDevice) && !req.session.ipadDevice) {
            req.session.ipadDevice = true; 
        }
        return next();
    }

    // 2. Recovery via Header or Query Param (Critical for Mobile/Biometric Redirects)
    ...
    if (externalSessionId) {
        console.log('🔄 Attempting session restoration from ID:', externalSessionId);
        
        req.sessionStore.get(externalSessionId, (err, sessionData) => {
            if (err || !sessionData || !sessionData.user) {
                console.log('❌ Restoration failed: Invalid session ID');
            ...
            // CRITICAL: Force Cookie Reset for the WebView
            const cookieOptions = {
                maxAge: 24 * 60 * 60 * 1000,
                httpOnly: false, // Allow JS access for mobile
                secure: false,   // Important for capacitor:// protocol
                sameSite: 'Lax',
                path: '/'
            };
            
            // Manually set the cookie header so the WebView accepts it
            res.cookie('solura.session', externalSessionId, cookieOptions);
            
            console.log('✅ Session successfully restored and cookie forced');
...
// CRITICAL: iOS/Capacitor Session Persistence Fix
app.use((req, res, next) => {
    ...
    if (isIOS || isCapacitor) {
        console.log('📱 iOS/Capacitor Device - Enhanced Session Handling');
        
        // Force session creation for iOS
        if (!req.session.initialized) {
            req.session.initialized = true;
            req.session.iosDevice = true;
            console.log('📱 iOS session initialized');
        }
        
        // Enhanced cookie settings for iOS
        res.cookie('solura.session', req.sessionID, {
            maxAge: 24 * 60 * 60 * 1000,
            httpOnly: false, // iOS needs JS access to cookies
            secure: false,   // iOS has issues with secure cookies in Capacitor
            sameSite: 'Lax', // Lax works better for iOS
            path: '/',
            domain: isProduction ? '.solura.uk' : undefined
        });
        
        // Add session ID to all responses
        res.setHeader('X-Session-ID', req.sessionID);
